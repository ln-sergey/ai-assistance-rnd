...
| Р8 | **Blind re-annotation выполняем, но без API-вызовов.** Conservative-аннотатор применяется к synth-dirty карточкам через Claude Code в локальной сессии (агент сам читает `prompts/annotate-conservative-v1.txt` и размечает каждую карточку, как в пилоте Sprint P4). Запрещено использовать любые целевые провайдеры (Yandex / GigaChat) и любые эталонные AI через API для подготовки или валидации тестовых данных — это hard rule всего проекта (см. ниже). |
...
1. `scripts/data/synthesize-validate.ts`:
   ...
   - Считает метрики:
     ...
     - **Слепой re-annotation — отдельный workflow, НЕ в этом скрипте.**
       ...
       эталонных AI через API для подготовки/валидации тестовых данных.
       Поэтому `synth:validate` НЕ вызывает LLM; никаких флагов
       `--blind-reannotate` / `--provider`. Сам blind re-annotation
       выполняется параллельным шагом — Claude Code в локальной сессии
       читает `prompts/annotate-conservative-v1.txt` и применяет к
       каждой synth-dirty карточке вручную, как обычная разметка
       (см. Sprint P4 / pipeline v2). Workflow + шаблон делегации —
...
1. `docs/synthetic-guide.md` (~100 строк):
   ...
   - **Blind re-annotation** — отдельный шаг workflow (НЕ скрипт).
     Claude Code в сессии применяет `prompts/annotate-conservative-v1.txt`
     к каждой synth-dirty карточке. Если target_rule_id не ловится —
     карточка пере-генерируется (Р11). Никаких API-вызовов: Yandex /
...
- **Не вызывать API любых эталонных AI (включая Claude через Anthropic
  API, OpenAI, и т.п.) для генерации/разметки/валидации тестовых
  данных.** На текущий момент весь pipeline подготовки данных — только
  через Claude Code в локальной сессии (агент сам читает промпты и
  заполняет файлы). Если в будущем понадобится batch-режим — отдельное
  обсуждение и явное снятие этого ограничения.
...
3. **Blind re-annotation.** → **Делаем, но без API.** Сам факт «прогнать
   conservative промпт по synth-dirty карточкам и проверить, что target
   ловится» — нужен. Но per Р8 + новое hard-rule: ни Yandex/GigaChat,
   ни любой другой эталонный AI через API не вызываются. Реализация —
   через Claude Code в локальной сессии (отдельный шаг workflow в
   `docs/synthetic-guide.md`). Скрипт `synth:validate` остаётся
   network-free.
