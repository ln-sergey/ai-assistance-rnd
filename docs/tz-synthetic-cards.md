...
реальных карточек: локальный AI-агент в интерактивной сессии (Claude
Code, Codex, Cursor, Aider или любой совместимый передовой агент)
запускает скрипт, видит дельту покрытия, заполняет pending-файлы из
шаблона по канонизированному промпту, коммитит, материализует cases.
...
| ID | Решение |
|----|---------|
| Р1 | Локальный AI-агент = любой передовой агент в интерактивной сессии (Claude Code, Codex, Cursor, Aider или совместимый). Никаких прямых API-batch-вызовов из скриптов проекта; pipeline идентичен существующему `annotations:scaffold → fill → commit` (см. Sprint P4 + pipeline v2). Конкретный агент пилотного прогона зафиксирован в `_meta.generator_model` каждой synth-карточки и в `annotator` записи `synthetic.json` (по умолчанию `local-ai-agent-session`, override через ENV `SYNTH_GENERATOR_MODEL` / `SYNTH_ANNOTATOR_LABEL`). |
...
| Р6 | `datasets/sources.config.json` пополняется записью `synthetic` с `kind: "synthetic"` и пустыми `target_total`/`seed`. Массовые операции `--all` и lifecycle-скрипты подхватывают синтетику автоматически (Variant A). |
| Р7 | Формат `case_id` для синтетики — `synth_txt05_001` (rule_id без дефиса, three-digit running number). Стабильнее под glob `synth_txt05_*.json`, чем форма с дефисом. |
| Р8 | **Blind re-annotation выполняем, но без прямых API-вызовов.** Conservative-аннотатор применяется к synth-dirty карточкам через локальный AI-агент в интерактивной сессии (агент сам читает `prompts/annotate-conservative-v1.txt` и размечает каждую карточку, как в пилоте Sprint P4). Запрещено использовать любые целевые провайдеры (Yandex / GigaChat) и любые эталонные AI через прямые API-вызовы для подготовки или валидации тестовых данных — это hard rule всего проекта (см. ниже). |
| Р9 | Тематика `clean-control` карточек — равномерно по тем же тематикам, что dirty (Variant A). «Пограничные» темы (детские/экстрим) — отдельный скоп после пилота. |
| Р10 | Объём первого прогона — 30 synth-кейсов по самым приоритетным правилам (преимущественно critical без покрытия), а не полная квота ~175 кейсов. Полная квота закрывается итеративно после проверки натуральности на пилоте. |
| Р11 | Если blind re-annotation показал, что conservative-аннотатор НЕ ловит синтетику — пере-генерить карточку с более явным нарушением (Variant A). «Hard case»-метрика на тонких нарушениях — отдельный скоп позже. |

## Текущее состояние (вход)

Снимок на 2026-04-27 (после повторного прогона `pnpm cases:generate` —
размеченных карточек стало больше, чем в `docs/real-cards-audit.md` от
2026-04-25; авторитетный источник теперь — фактический `datasets/cases/`,
а не статический отчёт):

- 122 реальных кейса: 91 clean + 31 dirty.
- 39 нарушений на **13 правилах**: TXT-01, TXT-03, TXT-04, TXT-07,
  TXT-19, TXT-20, TXT-21, TXT-22, TXT-23, TXT-24, TXT-25, TXT-26, TXT-33.
  Доминирует TXT-20 (16 hits — паттерн scantour), затем TXT-07 (6 hits —
  телефоны/контакты в карточках scantour), TXT-19 (3 — Afisha.ru в
  short_description).
- **22 правила из 35 — без единого реального примера.**
  - Critical без покрытия (8 из 9): TXT-05, TXT-08, TXT-09, TXT-11,
    TXT-14, TXT-15, TXT-16, TXT-17. TXT-07 уже закрыт scantour'ом.
  - High без покрытия (7 из 12): TXT-02, TXT-06, TXT-10, TXT-12,
    TXT-31, TXT-34, TXT-35.
  - Medium без покрытия (3 из 8): TXT-13, TXT-18, TXT-27.
  - Low без покрытия (4 из 6): TXT-28, TXT-29, TXT-30, TXT-32.
...
- `prompts/annotate-conservative-v1.txt` + `prompts/annotate-aggressive-v1.txt`
  существуют — будут использованы как «слепой re-annotation» в session
  (см. Р8: без API-вызовов).
...
- `pnpm cases:audit` на текущем состоянии (0 synthetic) показывает
  снимок из раздела «Текущее состояние» этого ТЗ: 39 hits на 13
  правилах, 22 правила без покрытия. Конкретные числа сверяются с
  фактическим содержимым `datasets/cases/real-{clean,dirty}/`, а не со
  статическим `docs/real-cards-audit.md` (тот не пересоберётся в этом
  спринте). Числа меняются по мере добавления разметки — `cases:audit`
...
1. `scripts/data/synthesize-validate.ts`:
   ...
   - Считает метрики:
     ...
     - **Слепой re-annotation — отдельный workflow, НЕ в этом скрипте.**
       Per Р8: запрещено использовать целевых провайдеров и любых
       эталонных AI через прямые API-вызовы для подготовки/валидации
       тестовых данных. Поэтому `synth:validate` НЕ вызывает LLM;
       никаких флагов `--blind-reannotate` / `--provider`. Сам blind
       re-annotation выполняется параллельным шагом — локальный
       AI-агент в интерактивной сессии читает
       `prompts/annotate-conservative-v1.txt` и применяет к каждой
       synth-dirty карточке вручную, как обычная разметка (см.
       Sprint P4 / pipeline v2). Workflow + шаблон делегации —
       в `docs/synthetic-guide.md` (этап 6).
   - Вывод:
...
- Длины полей: прогнать на real-карточках — все они должны попасть в
  собственный 5-95 перцентиль (по построению, sanity-check).
- В скрипте нет ни одного импорта `providers/`. `pnpm synth:validate`
...
1. `docs/synthetic-guide.md` (~100 строк):
   ...
   - **Blind re-annotation** — отдельный шаг workflow (НЕ скрипт).
     Локальный AI-агент в интерактивной сессии применяет
     `prompts/annotate-conservative-v1.txt` к каждой synth-dirty
     карточке. Если target_rule_id не ловится — карточка
     пере-генерируется (Р11). Никаких прямых API-вызовов: Yandex /
     GigaChat / эталонные AI через API запрещены (см. hard rules).
...
1. **Натуральность.** Если упустим, синтетика будет искусственно
   ...
   blind re-annotation в локальной сессии (без API), чёрный список
   маркеров, итеративный пересмотр промпта (`synthesize-card-vN.txt`).
...
- **Не вызывать целевых провайдеров (Yandex / GigaChat) для подготовки
  или валидации тестовых данных нигде** — ни в `synth:scaffold`, ни в
  `synth:commit`, ни в `synth:validate`, ни в любом другом
  pipeline-скрипте. Целевые провайдеры — это subject-под-тестом, их
  ответы нельзя примешивать к ground truth.
- **Не вызывать прямых API любых эталонных AI (включая Claude через
  Anthropic API, OpenAI, и т.п.) для генерации/разметки/валидации
  тестовых данных.** На текущий момент весь pipeline подготовки данных —
  только через локальный AI-агент в интерактивной сессии (Claude Code,
  Codex, Cursor, Aider или совместимый передовой агент): агент сам
  читает промпты и заполняет файлы, никаких batch-вызовов из скриптов
  проекта. Если в будущем понадобится batch-режим — отдельное обсуждение
  и явное снятие этого ограничения.
...
целевым провайдерам и эталонным AI для подготовки тестовых данных
запрещены.
...
1. **`datasets/sources.config.json` — добавлять synthetic?** → **Variant A.**
   Записать `synthetic` с `kind: "synthetic"` (или эквивалентным
   полем) и пустыми `target_total`/`seed`. Массовые операции `--all`
   подхватывают автоматически. См. Р6.

2. **Формат `case_id` для synthetic.** → **Variant A: `synth_txt05_001`.**
   Без дефиса в rule_id. Стабильнее под glob `synth_txt05_*.json`. См. Р7.

3. **Blind re-annotation.** → **Делаем, но без прямых API-вызовов.**
   Сам факт «прогнать conservative промпт по synth-dirty карточкам и
   проверить, что target ловится» — нужен. Но per Р8 + hard-rule:
   ни Yandex/GigaChat, ни любой другой эталонный AI через прямые API
   не вызываются. Реализация — через локальный AI-агент в интерактивной
   сессии (отдельный шаг workflow в `docs/synthetic-guide.md`). Скрипт
   `synth:validate` остаётся network-free.

4. **Тематика clean-control карточек.** → **Variant A.** Равномерно по
   тем же тематикам, что dirty. «Пограничные» темы (TXT-09/16/15) —
   отдельный скоп после пилота. См. Р9.

5. **Объём первого прогона.** → **30 кейсов на самые критические
   правила.** Проверить пайплайн и натуральность; масштабировать
   итеративно. См. Р10.

6. **Что делать, если conservative-аннотатор НЕ ловит synth-карточку?**
   → **Variant A: пере-генерить с более явным нарушением.** «Hard case»-
   метрика на тонких нарушениях — отдельный скоп позже. См. Р11.
