...
// Короткий отзыв, намеренно безобидный — smoke-тест не про качество
// классификации, а про «пайплайн собран, ответ пришёл, JSON валидный».
const SAMPLE_REVIEW = 'Экскурсия по Питеру понравилась. Гид знающий, группа небольшая.';

const TEXT_PROMPT = [
  {
    role: 'system',
    content:
      'Ты — модератор отзывов на маркетплейсе туристических активностей. ' +
      'Верни строго JSON вида {"violations":[],"verdict":"approve"|"needs_review"|"reject"}. ' +
      'Без комментариев и markdown.',
  },
  { role: 'user', content: `Отзыв для модерации:\n\n${SAMPLE_REVIEW}` },
];
...
const VISION_PROMPT = [
  {
    role: 'system',
    content:
      'Ты — модератор фотографий для маркетплейса экскурсий. Верни строго JSON ' +
      '{"violations":[],"verdict":"approve"|"needs_review"|"reject"}. Без markdown.',
  },
  {
    role: 'user',
    content: [
      { type: 'text', text: 'Проверь фото к экскурсии на допустимость.' },
      {
        type: 'image_url',
